...
function PublicPage() {
  ...
  useEffect(() => {
    if (isPastDate(date)) { setDate(dateInputValue(new Date())); setSlot(null); }
  }, [today, date]);
  const range = useMemo(() => ({ from: dateInputValue(today), to: dateInputValue(addDays(today, 13)) }), [today]);
